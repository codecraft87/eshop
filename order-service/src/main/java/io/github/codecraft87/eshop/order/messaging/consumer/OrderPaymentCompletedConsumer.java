...
public class OrderPaymentCompletedConsumer {
  ...
  private final EventIdempotency orderProcessedEventIdempotency;

  @RabbitListener(queues = OrderQueueConstants.ORDER_PAYMENT_COMPLETED_QUEUE)
  public void handlePaymentRequestedEvent(String payload) {
    ...
    if (paymentCompleted != null) {
      ...
      if (orderProcessedEventIdempotency.isEventProcessed(eventId)) {
        log.info("Duplicate event {} ignored", eventId);
        return;
      }
      orderService.updateOrderForPayment(paymentCompleted.orderId());
      orderProcessedEventIdempotency.saveProcessedEvent(eventId);
    }
  }
}
