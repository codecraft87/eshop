...
public class OrderMapper {
  public static OrderResponse getOrderResponse(Order order) {
    return OrderResponse.builder()
        ...
        .orderItems(order.getOrderItems().stream().map(OrderMapper::getOrderItemRequest).toList())
  ...
  public static Order getOrderEntity(OrderRequest orderRequest) {
    ...
    List<OrderItem> items =
        orderRequest.getOrderItems().stream().map(OrderMapper::getOrderItemEntity).toList();
    items.forEach(item -> item.setOrder(order));
    order.setOrderItems(items);
    return order;
  }

  public static OrderItemRequest getOrderItemRequest(OrderItem orderItem) {
  ...
  public static OrderItem getOrderItemEntity(OrderItemRequest orderItemDTO) {
